    ...
    public class StockService {
       ...
       private boolean todosLosAtributosCompletos(Product product) {
        ...
        public List<Product> EditaleData(DefaultTableModel model) {
            List<Product> ProductPersist = null;
        ...
        Product Product = new Product();
                    ...
                    for (int i=0;i<model.getRowCount();i++){
                        for(int j=0;j<model.getColumnCount();j++){
                            switch (j) {
                                case 0 -> Product.setId((Integer)model.getValueAt(j, j));   
                                case 1 -> Product.setProductName((String) model.getValueAt(i, j));
                                case 2 -> Product.setProductBlend((String) model.getValueAt(i, j));
                                case 3 -> Product.setCategory((String) model.getValueAt(i, j));
                                case 4 -> Product.setBuyingPrice((Double) model.getValueAt(i, j));
                                case 5 -> Product.setSellingPRice((Double) model.getValueAt(i, j));
                                case 6 -> Product.setStock((Integer) model.getValueAt(i, j));
                                case 7 -> Product.setStockWarning((Integer) model.getValueAt(i, j));
                                case 8 -> Product.setInterest((Double) model.getValueAt(i, j));
                                default -> {
                               }
                            } 
                             }
